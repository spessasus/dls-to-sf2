(()=>{var Bt=(o=>typeof require<"u"?require:typeof Proxy<"u"?new Proxy(o,{get:(A,e)=>(typeof require<"u"?require:A)[e]}):o)(function(o){if(typeof require<"u")return require.apply(this,arguments);throw Error('Dynamic require of "'+o+'" is not supported')});var F=class extends Uint8Array{currentIndex;constructor(A){super(A),this.currentIndex=0}};function JA(o){let A=o.reduce((I,a)=>I+a.length,0),e=new F(A),t=0;for(let I of o)e.set(I,t),t+=I.length;return e}function V(o,A,e=void 0,t=!0){if(e){let I=o.slice(o.currentIndex,o.currentIndex+A);return o.currentIndex+=A,new TextDecoder(e.replace(/[^\x20-\x7E]/g,"")).decode(I.buffer)}else{let I=!1,a="";for(let B=0;B<A;B++){let C=o[o.currentIndex++];if(!I){if(C<32||C>127){if(t){I=!0;continue}else if(C===0){I=!0;continue}}a+=String.fromCharCode(C)}}return a}}function rA(o,A,e=0){e>0&&A.length>e&&(A=A.slice(0,e));for(let t=0;t<A.length;t++)o[o.currentIndex++]=A.charCodeAt(t);if(e>A.length)for(let t=0;t<e-A.length;t++)o[o.currentIndex++]=0;return o}var II=!1,nI=!0,ke=!1;function x(...o){II&&console.info(...o)}function AA(...o){nI&&console.warn(...o)}function yA(...o){ke&&console.group(...o)}function dA(...o){ke&&console.groupCollapsed(...o)}function v(){ke&&console.groupEnd()}var l={warn:"color: orange;",unrecognized:"color: red;",info:"color: aqua;",recognized:"color: lime",value:"color: yellow; background-color: black;"};function d(o,A){let e=0;for(let t=0;t<A;t++)e|=o[o.currentIndex++]<<t*8;return e>>>0}function bA(o,A,e){for(let t=0;t<e;t++)o[o.currentIndex++]=A>>t*8&255}function k(o,A){o[o.currentIndex++]=A&255,o[o.currentIndex++]=A>>8}function nA(o,A){bA(o,A,4)}function RA(o,A){let e=A<<8|o;return e>32767?e-65536:e}function Ct(o){return o>127?o-256:o}var Z=class{constructor(A,e,t){this.header=A,this.size=e,this.chunkData=t}};function L(o,A=!0,e=!1){let t=V(o,4),I=d(o,4),a;return A&&(a=new F(o.buffer.slice(o.currentIndex,o.currentIndex+I))),(A||e)&&(o.currentIndex+=I),I%2!==0&&o[o.currentIndex]===0&&o.currentIndex++,new Z(t,I,a)}function X(o,A=void 0){let e=8+o.size;o.size%2!==0&&e++,A&&(e+=A.length);let t=new F(e);return A&&(t.set(A,t.currentIndex),t.currentIndex+=A.length),rA(t,o.header),nA(t,e-8-(A?.length||0)),t.set(o.chunkData,t.currentIndex),t}function aA(o,A){return o.find(e=>e.header!=="LIST"?!1:(e.chunkData.currentIndex=0,V(e.chunkData,4)===A))}var r={INVALID:-1,startAddrsOffset:0,endAddrOffset:1,startloopAddrsOffset:2,endloopAddrsOffset:3,startAddrsCoarseOffset:4,modLfoToPitch:5,vibLfoToPitch:6,modEnvToPitch:7,initialFilterFc:8,initialFilterQ:9,modLfoToFilterFc:10,modEnvToFilterFc:11,endAddrsCoarseOffset:12,modLfoToVolume:13,unused1:14,chorusEffectsSend:15,reverbEffectsSend:16,pan:17,unused2:18,unused3:19,unused4:20,delayModLFO:21,freqModLFO:22,delayVibLFO:23,freqVibLFO:24,delayModEnv:25,attackModEnv:26,holdModEnv:27,decayModEnv:28,sustainModEnv:29,releaseModEnv:30,keyNumToModEnvHold:31,keyNumToModEnvDecay:32,delayVolEnv:33,attackVolEnv:34,holdVolEnv:35,decayVolEnv:36,sustainVolEnv:37,releaseVolEnv:38,keyNumToVolEnvHold:39,keyNumToVolEnvDecay:40,instrument:41,reserved1:42,keyRange:43,velRange:44,startloopAddrsCoarseOffset:45,keyNum:46,velocity:47,initialAttenuation:48,reserved2:49,endloopAddrsCoarseOffset:50,coarseTune:51,fineTune:52,sampleID:53,sampleModes:54,reserved3:55,scaleTuning:56,exclusiveClass:57,overridingRootKey:58,unused5:59,endOper:60},w=[];w[r.startAddrsOffset]={min:0,max:32768,def:0};w[r.endAddrOffset]={min:-32768,max:32768,def:0};w[r.startloopAddrsOffset]={min:-32768,max:32768,def:0};w[r.endloopAddrsOffset]={min:-32768,max:32768,def:0};w[r.startAddrsCoarseOffset]={min:0,max:32768,def:0};w[r.modLfoToPitch]={min:-12e3,max:12e3,def:0};w[r.vibLfoToPitch]={min:-12e3,max:12e3,def:0};w[r.modEnvToPitch]={min:-12e3,max:12e3,def:0};w[r.initialFilterFc]={min:1500,max:13500,def:13500};w[r.initialFilterQ]={min:0,max:960,def:0};w[r.modLfoToFilterFc]={min:-12e3,max:12e3,def:0};w[r.modEnvToFilterFc]={min:-12e3,max:12e3,def:0};w[r.endAddrsCoarseOffset]={min:-32768,max:32768,def:0};w[r.modLfoToVolume]={min:-960,max:960,def:0};w[r.chorusEffectsSend]={min:0,max:1e3,def:0};w[r.reverbEffectsSend]={min:0,max:1e3,def:0};w[r.pan]={min:-500,max:500,def:0};w[r.delayModLFO]={min:-12e3,max:5e3,def:-12e3};w[r.freqModLFO]={min:-16e3,max:4500,def:0};w[r.delayVibLFO]={min:-12e3,max:5e3,def:-12e3};w[r.freqVibLFO]={min:-16e3,max:4500,def:0};w[r.delayModEnv]={min:-32768,max:5e3,def:-32768};w[r.attackModEnv]={min:-32768,max:8e3,def:-32768};w[r.holdModEnv]={min:-12e3,max:5e3,def:-12e3};w[r.decayModEnv]={min:-7200,max:8e3,def:-12e3};w[r.sustainModEnv]={min:0,max:1e3,def:0};w[r.releaseModEnv]={min:-7200,max:8e3,def:-12e3};w[r.keyNumToModEnvHold]={min:-1200,max:1200,def:0};w[r.keyNumToModEnvDecay]={min:-1200,max:1200,def:0};w[r.delayVolEnv]={min:-12e3,max:5e3,def:-12e3};w[r.attackVolEnv]={min:-12e3,max:8e3,def:-12e3};w[r.holdVolEnv]={min:-12e3,max:5e3,def:-12e3};w[r.decayVolEnv]={min:-12e3,max:8e3,def:-12e3};w[r.sustainVolEnv]={min:0,max:1440,def:0};w[r.releaseVolEnv]={min:-7200,max:8e3,def:-12e3};w[r.keyNumToVolEnvHold]={min:-1200,max:1200,def:0};w[r.keyNumToVolEnvDecay]={min:-1200,max:1200,def:0};w[r.startloopAddrsCoarseOffset]={min:-32768,max:32768,def:0};w[r.keyNum]={min:-1,max:127,def:-1};w[r.velocity]={min:-1,max:127,def:-1};w[r.initialAttenuation]={min:-250,max:1440,def:0};w[r.endloopAddrsCoarseOffset]={min:-32768,max:32768,def:0};w[r.coarseTune]={min:-120,max:120,def:0};w[r.fineTune]={min:-12700,max:12700,def:0};w[r.scaleTuning]={min:0,max:1200,def:100};w[r.exclusiveClass]={min:0,max:99999,def:0};w[r.overridingRootKey]={min:-1,max:127,def:-1};var y=class{generatorType=r.INVALID;generatorValue=0;constructor(A=r.INVALID,e=0){if(this.generatorType=A,e===void 0)throw new Error("No value provided.");let t=w[A];this.generatorValue=Math.round(e),t!==void 0&&(this.generatorValue=Math.max(t.min,Math.min(t.max,this.generatorValue)))}};function Qt(){let o=4;for(let t of this.instruments)o+=t.instrumentZones.reduce((I,a)=>(a.generators=a.generators.filter(B=>B.generatorType!==r.sampleID&&B.generatorType!==r.keyRange&&B.generatorType!==r.velRange),(a.velRange.max!==127||a.velRange.min!==0)&&a.generators.unshift({generatorType:r.velRange,generatorValue:a.velRange.max<<8|a.velRange.min}),(a.keyRange.max!==127||a.keyRange.min!==0)&&a.generators.unshift({generatorType:r.keyRange,generatorValue:a.keyRange.max<<8|a.keyRange.min}),a.isGlobal||a.generators.push({generatorType:r.sampleID,generatorValue:this.samples.indexOf(a.sample)}),a.generators.length*4+I),0);let A=new F(o),e=0;for(let t of this.instruments)for(let I of t.instrumentZones){I.generatorZoneStartIndex=e;for(let a of I.generators)k(A,a.generatorType),k(A,a.generatorValue),e++}return nA(A,0),X(new Z("igen",A.length,A))}function ht(o,A,e,t,I){let a=this.samples.map((u,S)=>{e&&u.compressSample(t,I);let f=u.getRawData();return x(`%cEncoded sample %c${S}. ${u.sampleName}%c of %c${this.samples.length}`,l.info,l.recognized,l.info,l.recognized),f}),B=this.samples.reduce((u,S,f)=>u+a[f].length+46,0),C=new F(B);this.samples.forEach((u,S)=>{let f=a[S],Q,c,N=f.length;u.isCompressed?(Q=C.currentIndex,c=Q+f.length):(Q=C.currentIndex/2,c=Q+f.length/2,N+=46),o.push(Q),C.set(f,C.currentIndex),C.currentIndex+=N,A.push(c)});let h=X(new Z("smpl",C.length,C),new F([115,100,116,97]));return X(new Z("LIST",h.length,h))}function ct(o,A){let t=new F(46*(this.samples.length+1));return this.samples.forEach((I,a)=>{rA(t,I.sampleName,20);let B=o[a];nA(t,B);let C=A[a];nA(t,C);let h=I.sampleLoopStartIndex+B,u=I.sampleLoopEndIndex+B;I.isCompressed&&(h-=B,u-=B),nA(t,h),nA(t,u),nA(t,I.sampleRate),t[t.currentIndex++]=I.samplePitch,t[t.currentIndex++]=I.samplePitchCorrection,k(t,I.sampleLink),k(t,I.sampleType)}),rA(t,"EOS",46),X(new Z("shdr",t.length,t))}function lt(){let o=10;for(let t of this.instruments)o+=t.instrumentZones.reduce((I,a)=>a.modulators.length*10+I,0);let A=new F(o),e=0;for(let t of this.instruments)for(let I of t.instrumentZones){I.modulatorZoneStartIndex=e;for(let a of I.modulators)k(A,a.sourceEnum),k(A,a.modulatorDestination),k(A,a.transformAmount),k(A,a.secondarySourceEnum),k(A,a.transformType),e++}return bA(A,0,10),X(new Z("imod",A.length,A))}function dt(){let o=this.instruments.reduce((a,B)=>B.instrumentZones.length*4+a,4),A=new F(o),e=0,t=0,I=0;for(let a of this.instruments){a.instrumentZoneIndex=e;for(let B of a.instrumentZones)B.zoneID=e,k(A,t),k(A,I),t+=B.generators.length,I+=B.modulators.length,e++}return k(A,t),k(A,I),X(new Z("ibag",A.length,A))}function ft(){let o=this.instruments.length*22+22,A=new F(o),e=0,t=0;for(let I of this.instruments)rA(A,I.instrumentName,20),k(A,e),e+=I.instrumentZones.length,I.instrumentID=t,t++;return rA(A,"EOI",20),k(A,e),X(new Z("inst",A.length,A))}function ut(){let o=4;for(let t of this.presets)o+=t.presetZones.reduce((I,a)=>(a.generators=a.generators.filter(B=>B.generatorType!==r.instrument&&B.generatorType!==r.keyRange&&B.generatorType!==r.velRange),(a.velRange.max!==127||a.velRange.min!==0)&&a.generators.unshift({generatorType:r.velRange,generatorValue:a.velRange.max<<8|a.velRange.min}),(a.keyRange.max!==127||a.keyRange.min!==0)&&a.generators.unshift({generatorType:r.keyRange,generatorValue:a.keyRange.max<<8|a.keyRange.min}),a.isGlobal||a.generators.push({generatorType:r.instrument,generatorValue:this.instruments.indexOf(a.instrument)}),a.generators.length*4+I),0);let A=new F(o),e=0;for(let t of this.presets)for(let I of t.presetZones){I.generatorZoneStartIndex=e;for(let a of I.generators)k(A,a.generatorType),k(A,a.generatorValue);e+=I.generators.length}return k(A,0),k(A,0),X(new Z("pgen",A.length,A))}function mt(){let o=10;for(let t of this.presets)o+=t.presetZones.reduce((I,a)=>a.modulators.length*10+I,0);let A=new F(o),e=0;for(let t of this.presets)for(let I of t.presetZones){I.modulatorZoneStartIndex=e;for(let a of I.modulators)k(A,a.sourceEnum),k(A,a.modulatorDestination),k(A,a.transformAmount),k(A,a.secondarySourceEnum),k(A,a.transformType),e++}return bA(A,0,10),X(new Z("pmod",A.length,A))}function pt(){let o=this.presets.reduce((a,B)=>B.presetZones.length*4+a,4),A=new F(o),e=0,t=0,I=0;for(let a of this.presets){a.presetZoneStartIndex=e;for(let B of a.presetZones)B.zoneID=e,k(A,t),k(A,I),t+=B.generators.length,I+=B.modulators.length,e++}return k(A,t),k(A,I),X(new Z("pbag",A.length,A))}function Dt(){let o=this.presets.length*38+38,A=new F(o),e=0;for(let t of this.presets)rA(A,t.presetName,20),k(A,t.program),k(A,t.bank),k(A,e),nA(A,t.library),nA(A,t.genre),nA(A,t.morphology),e+=t.presetZones.length;return rA(A,"EOP",20),k(A,0),k(A,0),k(A,e),nA(A,0),nA(A,0),nA(A,0),X(new Z("phdr",A.length,A))}var sI={compress:!1,compressionQuality:.5,compressionFunction:void 0};function St(o=sI){if(o.compress&&typeof o.compressionFunction!="function")throw new TypeError("No compression function supplied but compression enabled.");dA("%cSaving soundfont...",l.info),x(`%cCompression: %c${o?.compress||"false"}%c quality: %c${o?.compressionQuality||"none"}`,l.info,l.recognized,l.info,l.recognized),x("%cWriting INFO...",l.info);let A=[];this.soundFontInfo.ISFT="SpessaSynth",o?.compress&&(this.soundFontInfo.ifil="3.0");for(let[O,q]of Object.entries(this.soundFontInfo))if(O==="ifil"||O==="iver"){let g=parseInt(q.split(".")[0]),IA=parseInt(q.split(".")[1]),$=new F(4);k($,g),k($,IA),A.push(X(new Z(O,4,$)))}else if(O==="DMOD")A.push(X(new Z(O,q.length,q)));else{let g=new F(q.length);rA(g,q),A.push(X(new Z(O,q.length,g)))}let e=JA([new F([73,78,70,79]),...A]),t=X(new Z("LIST",e.length,e));x("%cWriting SDTA...",l.info);let I=[],a=[],B=ht.call(this,I,a,o?.compress,o?.compressionQuality??.5,o.compressionFunction);x("%cWriting PDTA...",l.info),x("%cWriting SHDR...",l.info);let C=ct.call(this,I,a);x("%cWriting IGEN...",l.info);let h=Qt.call(this);x("%cWriting IMOD...",l.info);let u=lt.call(this);x("%cWriting IBAG...",l.info);let S=dt.call(this);x("%cWriting INST...",l.info);let f=ft.call(this),Q=ut.call(this);x("%cWriting PMOD...",l.info);let c=mt.call(this);x("%cWriting PBAG...",l.info);let N=pt.call(this);x("%cWriting PHDR...",l.info);let Y=Dt.call(this),W=JA([new F([112,100,116,97]),Y,N,c,Q,f,S,u,h,C]),M=X(new Z("LIST",W.length,W));x("%cWriting the output file...",l.info);let J=JA([new F([115,102,98,107]),t,B,M]),T=X(new Z("RIFF",J.length,J));return x(`%cSaved succesfully! Final file size: %c${T.length}`,l.info,l.recognized),v(),T}var H={bankSelect:0,modulationWheel:1,breathController:2,footController:4,portamentoTime:5,dataEntryMsb:6,mainVolume:7,balance:8,pan:10,expressionController:11,effectControl1:12,effectControl2:13,generalPurposeController1:16,generalPurposeController2:17,generalPurposeController3:18,generalPurposeController4:19,lsbForControl0BankSelect:32,lsbForControl1ModulationWheel:33,lsbForControl2BreathController:34,lsbForControl4FootController:36,lsbForControl5PortamentoTime:37,lsbForControl6DataEntry:38,lsbForControl7MainVolume:39,lsbForControl8Balance:40,lsbForControl10Pan:42,lsbForControl11ExpressionController:43,lsbForControl12EffectControl1:44,lsbForControl13EffectControl2:45,sustainPedal:64,portamentoOnOff:65,sostenutoPedal:66,softPedal:67,legatoFootswitch:68,hold2Pedal:69,soundVariation:70,timbreHarmonicContent:71,releaseTime:72,attackTime:73,brightness:74,soundController6:75,soundController7:76,soundController8:77,soundController9:78,soundController10:79,generalPurposeController5:80,generalPurposeController6:81,generalPurposeController7:82,generalPurposeController8:83,portamentoControl:84,reverbDepth:91,tremoloDepth:92,chorusDepth:93,detuneDepth:94,phaserDepth:95,dataIncrement:96,dataDecrement:97,NRPNLsb:98,NRPNMsb:99,RPNLsb:100,RPNMsb:101,allSoundOff:120,resetAllControllers:121,localControlOnOff:122,allNotesOff:123,omniModeOff:124,omniModeOn:125,monoModeOn:126,polyModeOn:127};var sA={noController:0,noteOnVelocity:2,noteOnKeyNum:3,polyPressure:10,channelPressure:13,pitchWheel:14,pitchWheelRange:16,link:127},uA={linear:0,concave:1,convex:2,switch:3},K=class o{currentValue=0;constructor(A){this.sourceEnum=A.srcEnum,this.modulatorDestination=A.dest,this.secondarySourceEnum=A.secSrcEnum,this.transformAmount=A.amt,this.transformType=A.transform,this.modulatorDestination>58&&(this.modulatorDestination=r.INVALID),this.sourcePolarity=this.sourceEnum>>9&1,this.sourceDirection=this.sourceEnum>>8&1,this.sourceUsesCC=this.sourceEnum>>7&1,this.sourceIndex=this.sourceEnum&127,this.sourceCurveType=this.sourceEnum>>10&3,this.secSrcPolarity=this.secondarySourceEnum>>9&1,this.secSrcDirection=this.secondarySourceEnum>>8&1,this.secSrcUsesCC=this.secondarySourceEnum>>7&1,this.secSrcIndex=this.secondarySourceEnum&127,this.secSrcCurveType=this.secondarySourceEnum>>10&3,this.isEffectModulator=(this.sourceEnum===219||this.sourceEnum===221)&&this.secondarySourceEnum===0&&(this.modulatorDestination===r.reverbEffectsSend||this.modulatorDestination===r.chorusEffectsSend)}static copy(A){return new o({srcEnum:A.sourceEnum,secSrcEnum:A.secondarySourceEnum,transform:A.transformType,amt:A.transformAmount,dest:A.modulatorDestination})}static isIdentical(A,e){return A.sourceEnum===e.sourceEnum&&A.modulatorDestination===e.modulatorDestination&&A.secondarySourceEnum===e.secondarySourceEnum&&A.transformType===e.transformType}sumTransform(A){return new o({srcEnum:this.sourceEnum,secSrcEnum:this.secondarySourceEnum,dest:this.modulatorDestination,transform:this.transformType,amt:this.transformAmount+A.transformAmount})}debugString(){function A(I,a){return Object.keys(I).find(B=>I[B]===a)}let e=A(uA,this.sourceCurveType);e+=this.sourcePolarity===0?" unipolar ":" bipolar ",e+=this.sourceDirection===0?"forwards ":"backwards ",this.sourceUsesCC?e+=A(H,this.sourceIndex):e+=A(sA,this.sourceIndex);let t=A(uA,this.secSrcCurveType);return t+=this.secSrcPolarity===0?" unipolar ":" bipolar ",t+=this.secSrcCurveType===0?"forwards ":"backwards ",this.secSrcUsesCC?t+=A(H,this.secSrcIndex):t+=A(sA,this.secSrcIndex),`Modulator:
        Source: ${e}
        Secondary source: ${t}
        Destination: ${A(r,this.modulatorDestination)}
        Trasform amount: ${this.transformAmount}
        Transform type: ${this.transformType}
        

`}},Re=960,Ge=uA.concave;function fA(o,A,e,t,I){return o<<10|A<<9|e<<8|t<<7|I}var ie=[new K({srcEnum:fA(Ge,0,1,0,sA.noteOnVelocity),dest:r.initialAttenuation,amt:Re,secSrcEnum:0,transform:0}),new K({srcEnum:129,dest:r.vibLfoToPitch,amt:50,secSrcEnum:0,transform:0}),new K({srcEnum:fA(Ge,0,1,1,H.mainVolume),dest:r.initialAttenuation,amt:Re,secSrcEnum:0,transform:0}),new K({srcEnum:13,dest:r.vibLfoToPitch,amt:50,secSrcEnum:0,transform:0}),new K({srcEnum:526,dest:r.fineTune,amt:12700,secSrcEnum:16,transform:0}),new K({srcEnum:650,dest:r.pan,amt:500,secSrcEnum:0,transform:0}),new K({srcEnum:fA(Ge,0,1,1,H.expressionController),dest:r.initialAttenuation,amt:Re,secSrcEnum:0,transform:0}),new K({srcEnum:219,dest:r.reverbEffectsSend,amt:200,secSrcEnum:0,transform:0}),new K({srcEnum:221,dest:r.chorusEffectsSend,amt:200,secSrcEnum:0,transform:0}),new K({srcEnum:fA(uA.linear,0,0,0,sA.polyPressure),dest:r.vibLfoToPitch,amt:50,secSrcEnum:0,transform:0}),new K({srcEnum:fA(uA.linear,0,0,1,H.tremoloDepth),dest:r.modLfoToVolume,amt:24,secSrcEnum:0,transform:0}),new K({srcEnum:fA(uA.linear,1,0,1,H.releaseTime),dest:r.releaseVolEnv,amt:1200,secSrcEnum:0,transform:0}),new K({srcEnum:fA(uA.linear,1,0,1,H.brightness),dest:r.initialFilterFc,amt:6e3,secSrcEnum:0,transform:0}),new K({srcEnum:fA(uA.linear,1,0,1,H.timbreHarmonicContent),dest:r.initialFilterQ,amt:250,secSrcEnum:0,transform:0})];var GA=class o{constructor(A=void 0){this.soundFontInfo={},this.presets=[],this.samples=[],this.instruments=[],this.defaultModulators=ie.map(e=>K.copy(e)),A?.presets&&(this.presets.push(...A.presets),this.soundFontInfo=A.info)}static mergeSoundfonts(...A){let e=A.shift(),t=e.presets;for(;A.length;)A.shift().presets.forEach(a=>{t.find(B=>B.bank===a.bank&&B.program===a.program)===void 0&&t.push(a)});return new o({presets:t,info:e.soundFontInfo})}removeUnusedElements(){this.instruments.forEach(A=>{A.useCount<1&&A.instrumentZones.forEach(e=>{e.isGlobal||e.sample.useCount--})}),this.instruments=this.instruments.filter(A=>A.useCount>0),this.samples=this.samples.filter(A=>A.useCount>0)}deleteInstrument(A){if(A.useCount>0)throw new Error(`Cannot delete an instrument that has ${A.useCount} usages.`);this.instruments.splice(this.instruments.indexOf(A),1),A.deleteInstrument(),this.removeUnusedElements()}deletePreset(A){A.deletePreset(),this.presets.splice(this.presets.indexOf(A),1),this.removeUnusedElements()}deleteSample(A){if(A.useCount>0)throw new Error(`Cannot delete sample that has ${A.useCount} usages.`);this.samples.splice(this.samples.indexOf(A),1),this.removeUnusedElements()}setSampleIDOffset(A){this.presets.forEach(e=>e.sampleIDOffset=A)}getPresetNoFallback(A,e,t=!1){let I=this.presets.find(a=>a.bank===A&&a.program===e);if(I)return I;if(t!==!1)return A===128?this.presets.find(a=>a.bank===128):this.presets.find(a=>a.program===e)}getPreset(A,e){let t=this.presets.find(I=>I.bank===A&&I.program===e);return t||(A===128?(t=this.presets.find(I=>I.bank===128&&I.program===e),t||(t=this.presets.find(I=>I.bank===128))):t=this.presets.find(I=>I.program===e&&I.bank!==128),t&&AA(`%cPreset ${A}.${e} not found. Replaced with %c${t.presetName} (${t.bank}.${t.program})`,l.warn,l.recognized)),t||(AA(`Preset ${e} not found. Defaulting to`,this.presets[0].presetName),t=this.presets[0]),t}getPresetByName(A){let e=this.presets.find(t=>t.presetName===A);return e||(AA("Preset not found. Defaulting to:",this.presets[0].presetName),e=this.presets[0]),e}parsingError(A){throw new Error(`SF parsing error: ${A} The file may be corrupted.`)}};GA.prototype.write=St;function yt(o){dA("%cLoading instruments...",l.info);for(let A=0;A<this.instrumentAmount;A++)this.readDLSInstrument(L(o.chunkData));v()}var qA=class{constructor(A){this.presetName="",this.program=0,this.bank=0,this.presetZones=[],this.sampleIDOffset=0,this.foundSamplesAndGenerators=[];for(let e=0;e<128;e++)this.foundSamplesAndGenerators[e]=[];this.library=0,this.genre=0,this.morphology=0,this.defaultModulators=A}deletePreset(){this.presetZones.forEach(A=>A.deleteZone()),this.presetZones.length=0}deleteZone(A){this.presetZones[A].deleteZone(),this.presetZones.splice(A,1)}preload(A,e){for(let t=A;t<e+1;t++)for(let I=0;I<128;I++)this.getSamplesAndGenerators(t,I).forEach(a=>{a.sample.isSampleLoaded||a.sample.getAudioData()})}preloadSpecific(A,e){this.getSamplesAndGenerators(A,e).forEach(t=>{t.sample.isSampleLoaded||t.sample.getAudioData()})}getSamplesAndGenerators(A,e){let t=this.foundSamplesAndGenerators[A][e];if(t)return t;if(this.presetZones.length<1)return[];function I(f,Q,c){return c>=f&&c<=Q}function a(f,Q){f.push(...Q.filter(c=>!f.find(N=>N.generatorType===c.generatorType)))}function B(f,Q){f.push(...Q.filter(c=>!f.find(N=>K.isIdentical(c,N))))}let C=[],h=this.presetZones[0].isGlobal?[...this.presetZones[0].generators]:[],u=this.presetZones[0].isGlobal?[...this.presetZones[0].modulators]:[];return this.presetZones.filter(f=>I(f.keyRange.min,f.keyRange.max,A)&&I(f.velRange.min,f.velRange.max,e)&&!f.isGlobal).forEach(f=>{if(f.instrument.instrumentZones.length<1)return;let Q=f.generators,c=f.modulators,N=f.instrument.instrumentZones[0].isGlobal?[...f.instrument.instrumentZones[0].generators]:[],Y=f.instrument.instrumentZones[0].isGlobal?[...f.instrument.instrumentZones[0].modulators]:[];f.instrument.instrumentZones.filter(M=>I(M.keyRange.min,M.keyRange.max,A)&&I(M.velRange.min,M.velRange.max,e)&&!M.isGlobal).forEach(M=>{let J=[...M.generators],T=[...M.modulators];a(Q,h),a(J,N),B(c,u),B(T,Y),B(T,this.defaultModulators);let O=[...T];for(let q=0;q<c.length;q++){let g=c[q],IA=O.findIndex($=>K.isIdentical(g,$));IA!==-1?O[IA]=O[IA].sumTransform(g):O.push(g)}C.push({instrumentGenerators:J,presetGenerators:Q,modulators:O,sample:M.sample,sampleID:M.generators.find(q=>q.generatorType===r.sampleID).generatorValue})})}),this.foundSamplesAndGenerators[A][e]=C,C}};var zA=class{velRange={min:0,max:127};keyRange={min:0,max:127};isGlobal=!1;generators=[];modulators=[]};var xA=class extends zA{sample=void 0;useCount=0;deleteZone(){this.useCount--,!this.isGlobal&&this.sample.useCount--}},TA=class extends zA{instrument=void 0;deleteZone(){this.isGlobal||this.instrument.removeUseCount()}};var vA=class{constructor(){this.instrumentName="",this.instrumentZones=[],this._useCount=0}get useCount(){return this._useCount}addUseCount(){this._useCount++,this.instrumentZones.forEach(A=>A.useCount++)}removeUseCount(){this._useCount--;for(let A=0;A<this.instrumentZones.length;A++)this.safeDeleteZone(A)&&A--}deleteInstrument(){this.instrumentZones.forEach(A=>A.deleteZone()),this.instrumentZones.length=0}safeDeleteZone(A){return this.instrumentZones[A].useCount--,this.instrumentZones[A].useCount<1?(this.deleteZone(A),!0):!1}deleteZone(A){this.instrumentZones[A].deleteZone(),this.instrumentZones.splice(A,1)}};var re=class extends qA{constructor(A,e){super(ie),this.program=e&127,this.bank=A>>8&127,A>>31&&(this.bank=128),this.DLSInstrument=new vA,this.DLSInstrument.addUseCount();let I=new TA;I.instrument=this.DLSInstrument,this.presetZones=[I]}};function wt(o){this.verifyHeader(o,"LIST"),this.verifyText(V(o.chunkData,4),"ins ");let A=[];for(;o.chunkData.length>o.chunkData.currentIndex;)A.push(L(o.chunkData));let e=A.find(c=>c.header==="insh");if(!e)throw v(),new Error("No instrument header!");let t=d(e.chunkData,4),I=d(e.chunkData,4),a=d(e.chunkData,4),B=new re(I,a),C="unnamedPreset",h=aA(A,"INFO");if(h){let c=L(h.chunkData);for(;c.header!=="INAM";)c=L(h.chunkData);C=V(c.chunkData,c.chunkData.length).trim()}B.presetName=C,B.DLSInstrument.instrumentName=C,yA(`%cParsing %c"${C}"%c...`,l.info,l.recognized,l.info);let u=aA(A,"lrgn");if(!u)throw v(),new Error("No region list!");let S=new xA;S.isGlobal=!0;let f=aA(A,"lart"),Q=aA(A,"lar2");this.readLart(f,Q,S),B.DLSInstrument.instrumentZones.push(S);for(let c=0;c<t;c++){let N=L(u.chunkData);this.verifyHeader(N,"LIST");let Y=V(N.chunkData,4);Y!=="rgn "&&Y!=="rgn2"&&(v(),this.parsingError(`Invalid DLS region! Expected "rgn " or "rgn2" got "${Y}"`));let W=this.readRegion(N);W&&B.DLSInstrument.instrumentZones.push(W)}this.presets.push(B),this.instruments.push(B.DLSInstrument),v()}var p={none:0,gain:1,reserved:2,pitch:3,pan:4,keyNum:5,chorusSend:128,reverbSend:129,modLfoFreq:260,modLfoDelay:261,vibLfoFreq:276,vibLfoDelay:277,volEnvAttack:518,volEnvDecay:519,volEnvRelease:521,volEnvSustain:522,volEnvDelay:523,volEnvHold:524,modEnvAttack:778,modEnvDecay:779,modEnvRelease:781,modEnvSustain:782,modEnvDelay:783,modEnvHold:784,filterCutoff:1280,filterQ:1281};var b={none:0,modLfo:1,velocity:2,keyNum:3,volEnv:4,modEnv:5,pitchWheel:6,polyPressure:7,channelPressure:8,vibratoLfo:9,modulationWheel:129,volume:135,pan:138,expression:139,chorus:219,reverb:221,pitchWheelRange:256,fineTune:257,coarseTune:258};function Ft(o){let A,e=!1;switch(o){default:case b.modLfo:case b.vibratoLfo:case b.coarseTune:case b.fineTune:case b.modEnv:return;case b.keyNum:A=sA.noteOnKeyNum;break;case b.none:A=sA.noController;break;case b.modulationWheel:A=H.modulationWheel,e=!0;break;case b.pan:A=H.pan,e=!0;break;case b.reverb:A=H.reverbDepth,e=!0;break;case b.chorus:A=H.chorusDepth,e=!0;break;case b.expression:A=H.expressionController,e=!0;break;case b.volume:A=H.mainVolume,e=!0;break;case b.velocity:A=sA.noteOnVelocity;break;case b.polyPressure:A=sA.polyPressure;break;case b.channelPressure:A=sA.channelPressure;break;case b.pitchWheel:A=sA.pitchWheel;break;case b.pitchWheelRange:A=sA.pitchWheelRange;break}if(A===void 0)throw new Error(`Unknown DLS Source: ${o}`);return{enum:A,isCC:e}}function gI(o,A){switch(o){default:case p.none:return;case p.pan:return r.pan;case p.gain:return{gen:r.initialAttenuation,newAmount:A*-1};case p.pitch:return r.fineTune;case p.keyNum:return r.overridingRootKey;case p.volEnvDelay:return r.delayVolEnv;case p.volEnvAttack:return r.attackVolEnv;case p.volEnvHold:return r.holdVolEnv;case p.volEnvDecay:return r.decayVolEnv;case p.volEnvSustain:return{gen:r.sustainVolEnv,newAmount:1e3-A};case p.volEnvRelease:return r.releaseVolEnv;case p.modEnvDelay:return r.delayModEnv;case p.modEnvAttack:return r.attackModEnv;case p.modEnvHold:return r.holdModEnv;case p.modEnvDecay:return r.decayModEnv;case p.modEnvSustain:return{gen:r.sustainModEnv,newAmount:(1e3-A)/10};case p.modEnvRelease:return r.releaseModEnv;case p.filterCutoff:return r.initialFilterFc;case p.filterQ:return r.initialFilterQ;case p.chorusSend:return r.chorusEffectsSend;case p.reverbSend:return r.reverbEffectsSend}}function oI(o,A){return o===b.vibratoLfo&&A===p.pitch?r.vibLfoToPitch:o===b.modLfo&&A===p.pitch?r.modLfoToPitch:o===b.modLfo&&A===p.filterCutoff?r.modLfoToFilterFc:o===b.modLfo&&A===p.gain?r.modLfoToVolume:o===b.modEnv&&A===p.filterCutoff?r.modEnvToFilterFc:o===b.modEnv&&A===p.pitch?r.modEnvToPitch:void 0}function kt(o,A,e,t,I){let a=oI(o,e),B,C,h=!1,u=!1,S=I;if(a===void 0){let M=gI(e,I);if(M===void 0||(B=M,M.newAmount!==void 0&&(S=M.newAmount,B=M.gen),C=Ft(o),C===void 0))return}else B=a,h=!0,C={enum:sA.noController,isCC:!1},u=!0;let f=Ft(A);if(f===void 0)return;let Q;if(u)Q=0;else{let M=t&15,J=t>>10&15;J===uA.linear&&M!==uA.linear&&(J=M);let T=t>>14&1,O=t>>15&1;B===r.initialAttenuation&&I>0&&(O=!O),Q=fA(J,T,O,C.isCC,C.enum)}let c=t>>4&15,N=t>>8&1,Y=t>>9&1,W=fA(c,N,Y,f.isCC,f.enum);if(h){let M=W;W=Q,Q=M}return new K({srcEnum:Q,secSrcEnum:W,dest:B,transform:0,amt:S})}function xe(o,A){let e=o.chunkData,t=[],I=[];d(e,4);let a=d(e,4);for(let B=0;B<a;B++){let C=d(e,2),h=d(e,2),u=d(e,2),S=d(e,2),Q=(d(e,4)|0)>>16;if(C===0&&h===0&&S===0){let c;switch(u){case p.pan:c=new y(r.pan,Q);break;case p.gain:c=new y(r.initialAttenuation,-Q*10/.4);break;case p.filterCutoff:c=new y(r.initialFilterFc,Q);break;case p.filterQ:c=new y(r.initialFilterQ,Q);break;case p.modLfoFreq:c=new y(r.freqModLFO,Q);break;case p.modLfoDelay:c=new y(r.delayModLFO,Q);break;case p.vibLfoFreq:c=new y(r.freqVibLFO,Q);break;case p.vibLfoDelay:c=new y(r.delayVibLFO,Q);break;case p.volEnvDelay:c=new y(r.delayVolEnv,Q);break;case p.volEnvAttack:c=new y(r.attackVolEnv,Q);break;case p.volEnvHold:c=new y(r.holdVolEnv,Q);break;case p.volEnvDecay:c=new y(r.decayVolEnv,Q);break;case p.volEnvRelease:c=new y(r.releaseVolEnv,Q);break;case p.volEnvSustain:let N=(1e3-Q)/10;c=new y(r.sustainVolEnv,N*10);break;case p.modEnvDelay:c=new y(r.delayModEnv,Q);break;case p.modEnvAttack:c=new y(r.attackModEnv,Q);break;case p.modEnvHold:c=new y(r.holdModEnv,Q);break;case p.modEnvDecay:c=new y(r.decayModEnv,Q);break;case p.modEnvRelease:c=new y(r.releaseModEnv,Q);break;case p.modEnvSustain:let Y=1e3-Q;c=new y(r.sustainModEnv,Y);break;case p.reverbSend:c=new y(r.reverbEffectsSend,Q);break;case p.chorusSend:c=new y(r.chorusEffectsSend,Q);break;case p.pitch:let W=Math.floor(Q/100),M=Math.floor(Q-W*100);c=new y(r.fineTune,M),t.push(new y(r.coarseTune,W));break}c&&t.push(c)}else{let c=!0;if(h===b.none)if(C===b.modLfo&&u===p.pitch)t.push(new y(r.modLfoToPitch,Q));else if(C===b.modLfo&&u===p.gain)t.push(new y(r.modLfoToVolume,Q));else if(C===b.modLfo&&u===p.filterCutoff)t.push(new y(r.modLfoToFilterFc,Q));else if(C===b.vibratoLfo&&u===p.pitch)t.push(new y(r.vibLfoToPitch,Q));else if(C===b.modEnv&&u===p.pitch)t.push(new y(r.modEnvToPitch,Q));else if(C===b.modEnv&&u===p.filterCutoff)t.push(new y(r.modEnvToFilterFc,Q));else if(C===b.keyNum&&u===p.volEnvHold){t.push(new y(r.keyNumToVolEnvHold,Q/-128));let N=Math.round(60/128*Q);t.forEach(Y=>{Y.generatorType===r.holdVolEnv&&(Y.generatorValue+=N)})}else if(C===b.keyNum&&u===p.volEnvDecay){t.push(new y(r.keyNumToVolEnvDecay,Q/-128));let N=Math.round(60/128*Q);t.forEach(Y=>{Y.generatorType===r.decayVolEnv&&(Y.generatorValue+=N)})}else if(C===b.keyNum&&u===p.modEnvHold){t.push(new y(r.keyNumToModEnvHold,Q/-128));let N=Math.round(60/128*Q);t.forEach(Y=>{Y.generatorType===r.holdModEnv&&(Y.generatorValue+=N)})}else if(C===b.keyNum&&u===p.modEnvDecay){t.push(new y(r.keyNumToModEnvDecay,Q/-128));let N=Math.round(60/128*Q);t.forEach(Y=>{Y.generatorType===r.decayModEnv&&(Y.generatorValue+=N)})}else c=!1;else c=!1;if(c===!1){let N=kt(C,h,u,S,Q);N?(I.push(N),x("%cSucceeded converting to SF2 Modulator!",l.recognized)):AA("Failed converting to SF2 Modulator!")}}}return I.find(B=>B.modulatorDestination===r.reverbEffectsSend)===void 0&&I.push(new K({srcEnum:219,dest:r.reverbEffectsSend,amt:1e3,secSrcEnum:0,transform:0})),I.find(B=>B.modulatorDestination===r.chorusEffectsSend)===void 0&&I.push(new K({srcEnum:221,dest:r.chorusEffectsSend,amt:1e3,secSrcEnum:0,transform:0})),A&&I.push(new K({srcEnum:129,dest:r.vibLfoToPitch,amt:0,secSrcEnum:0,transform:0}),new K({srcEnum:13,dest:r.vibLfoToPitch,amt:0,secSrcEnum:0,transform:0})),{modulators:I,generators:t}}function Rt(o,A,e){if(o)for(;o.chunkData.currentIndex<o.chunkData.length;){let t=L(o.chunkData);this.verifyHeader(t,"art1");let I=xe(t,!0);e.generators.push(...I.generators),e.modulators.push(...I.modulators)}if(A)for(;A.chunkData.currentIndex<A.chunkData.length;){let t=L(A.chunkData);this.verifyHeader(t,"art2");let I=xe(t,!1);e.generators.push(...I.generators),e.modulators.push(...I.modulators)}}var ae=class extends xA{constructor(A,e){super(),this.keyRange=A,this.velRange=e,this.isGlobal=!0}setWavesample(A,e,t,I,a,B,C){e!==0&&this.generators.push(new y(r.sampleModes,e)),this.generators.push(new y(r.initialAttenuation,A)),this.isGlobal=!1,C-=a.samplePitchCorrection;let h=Math.trunc(C/100);h!==0&&this.generators.push(new y(r.coarseTune,h));let u=C-h*100;u!==0&&this.generators.push(new y(r.fineTune,u));let S=t.start-a.sampleLoopStartIndex,f=t.end-a.sampleLoopEndIndex;if(S!==0){let Q=S%32768;this.generators.push(new y(r.startloopAddrsOffset,Q));let c=Math.trunc(S/32768);c!==0&&this.generators.push(new y(r.startloopAddrsCoarseOffset,c))}if(f!==0){let Q=f%32768;this.generators.push(new y(r.endloopAddrsOffset,Q));let c=Math.trunc(f/32768);c!==0&&this.generators.push(new y(r.endloopAddrsCoarseOffset,c))}I!==a.samplePitch&&this.generators.push(new y(r.overridingRootKey,I)),this.generators.push(new y(r.sampleID,B)),this.sample=a,a.useCount++}};function Gt(o){let A=[];for(;o.chunkData.length>o.chunkData.currentIndex;)A.push(L(o.chunkData));let e=A.find($=>$.header==="rgnh"),t=d(e.chunkData,2),I=d(e.chunkData,2),a=d(e.chunkData,2),B=d(e.chunkData,2),C=new ae({min:t,max:I},{min:a,max:B});d(e.chunkData,2);let h=d(e.chunkData,2);h!==0&&C.generators.push(new y(r.exclusiveClass,h));let u=aA(A,"lart"),S=aA(A,"lar2");this.readLart(u,S,C),C.isGlobal=!1;let f=A.find($=>$.header==="wsmp");d(f.chunkData,4);let Q=d(f.chunkData,2),c=RA(f.chunkData[f.chunkData.currentIndex++],f.chunkData[f.chunkData.currentIndex++]),Y=(d(f.chunkData,4)|0)/-655360;d(f.chunkData,4);let W=d(f.chunkData,4),M,J={start:0,end:0};if(W===0)M=0;else{d(f.chunkData,4),d(f.chunkData,4)===0?M=1:M=3,J.start=d(f.chunkData,4);let mA=d(f.chunkData,4);J.end=J.start+mA}let T=A.find($=>$.header==="wlnk");if(T===void 0)return;d(T.chunkData,2),d(T.chunkData,2),d(T.chunkData,4);let O=d(T.chunkData,4),q=this.samples[O];if(q===void 0)throw new Error("Invalid sample ID!");let IA=(Y||q.sampleDbAttenuation)*10/.4;return C.setWavesample(IA,M,J,Q,q,O,c),C}var KA=class{constructor(A,e,t,I,a,B,C,h){this.sampleName=A,this.sampleRate=e,this.samplePitch=t,this.samplePitchCorrection=I,this.sampleLink=a,this.sampleType=B,this.sampleLoopStartIndex=C,this.sampleLoopEndIndex=h,this.isCompressed=(B&16)>0,this.compressedData=void 0,this.useCount=0}getRawData(){let A=new Error("Not implemented");throw A.name="NotImplementedError",A}compressSample(A,e){if(!this.isCompressed)try{this.compressedData=e([this.getAudioData()],1,this.sampleRate,A),this.sampleType|=16,this.isCompressed=!0}catch{AA(`Failed to compress ${this.sampleName}. Leaving as uncompressed!`),this.isCompressed=!1,this.compressedData=void 0,this.sampleType&=-17}}getAudioData(){let A=new Error("Not implemented");throw A.name="NotImplementedError",A}};var Ee=class extends KA{sampleDbAttenuation;sampleData;constructor(A,e,t,I,a,B,C,h){super(A,e,t,I,0,1,a,B-1),this.sampleData=C,this.sampleDbAttenuation=h}getAudioData(){return this.sampleData}getRawData(){if(this.isCompressed){if(!this.compressedData)throw new Error("Compressed but no data?? This shouldn't happen!!");return this.compressedData}let A=new Uint8Array(this.sampleData.length*2);for(let e=0;e<this.sampleData.length;e++){let t=Math.floor(this.sampleData[e]*32767);A[e*2]=t&255,A[e*2+1]=t>>8&255}return A}};var xt={PCM:1,ALAW:6};function iI(o,A){let e=Math.pow(2,A*8-1),t=Math.pow(2,A*8),I,a=!1;A===1?(I=255,a=!0):I=e;let B=o.size/A,C=new Float32Array(B);for(let h=0;h<C.length;h++){let u=d(o.chunkData,A);a?C[h]=u/I-.5:(u>=e&&(u-=t),C[h]=u/I)}return C}function rI(o,A){let e=o.size/A,t=new Float32Array(e);for(let I=0;I<t.length;I++){let a=d(o.chunkData,A),B=a^85;B&=127;let C=B>>4,h=B&15;C>0&&(h+=16),h=(h<<4)+8,C>1&&(h=h<<C-1);let u=a>127?h:-h;t[I]=u/32678}return t}function Mt(o){dA("%cLoading Wave samples...",l.recognized);let A=0;for(;o.chunkData.currentIndex<o.chunkData.length;){let e=L(o.chunkData);this.verifyHeader(e,"LIST"),this.verifyText(V(e.chunkData,4),"wave");let t=[];for(;e.chunkData.currentIndex<e.chunkData.length;)t.push(L(e.chunkData));let I=t.find(q=>q.header==="fmt ");if(!I)throw new Error("No fmt chunk in the wave file!");let a=d(I.chunkData,2),B=d(I.chunkData,2);if(B!==1)throw new Error(`Only mono samples are supported. Fmt reports ${B} channels`);let C=d(I.chunkData,4);d(I.chunkData,4),d(I.chunkData,2);let u=d(I.chunkData,2)/8,S=!1,f=t.find(q=>q.header==="data");f||this.parsingError("No data chunk in the WAVE chunk!");let Q;switch(a){default:S=!0,Q=new Float32Array(f.size/u);break;case xt.PCM:Q=iI(f,u);break;case xt.ALAW:Q=rI(f,u);break}let c=60,N=0,Y=0,W=Q.length-1,M=0,J=t.find(q=>q.header==="wsmp");if(J){if(d(J.chunkData,4),c=d(J.chunkData,2),N=RA(J.chunkData[J.chunkData.currentIndex++],J.chunkData[J.chunkData.currentIndex++]),M=(d(J.chunkData,4)|0)/-655360,d(J.chunkData,4),d(J.chunkData,4)===1){d(J.chunkData,8),Y=d(J.chunkData,4);let IA=d(J.chunkData,4);W=Y+IA}}else AA("No wsmp chunk in wave... using sane defaults.");let T=aA(t,"INFO"),O=`Unnamed ${A}`;if(T){let q=L(T.chunkData);for(;q.header!=="INAM"&&T.chunkData.currentIndex<T.chunkData.length;)q=L(T.chunkData);q.header==="INAM"&&(O=V(q.chunkData,q.size).trim())}S&&console.error(`Failed to load '${O}': Unsupported format: (${a})`),this.samples.push(new Ee(O,C,c,N,Y,Q.length,Q,M)),A++,x(`%cLoaded sample %c${O}`,l.info,l.recognized)}v()}var wA=class extends GA{constructor(A){super(),this.dataArray=new F(A),yA("%cParsing DLS...",l.info),this.dataArray||(v(),this.parsingError("No data provided!"));let e=L(this.dataArray,!1);this.verifyHeader(e,"riff"),this.verifyText(V(this.dataArray,4).toLowerCase(),"dls ");let t=[];for(;this.dataArray.currentIndex<this.dataArray.length;)t.push(L(this.dataArray));this.soundFontInfo.ifil="2.1",this.soundFontInfo.isng="EMU8000",this.soundFontInfo.INAM="Unnamed DLS",this.soundFontInfo.IENG="Unknown",this.soundFontInfo.IPRD="SpessaSynth DLS",this.soundFontInfo.ICRD=new Date().toDateString();let I=aA(t,"INFO");if(I)for(;I.chunkData.currentIndex<I.chunkData.length;){let h=L(I.chunkData);this.soundFontInfo[h.header]=V(h.chunkData,h.size)}this.soundFontInfo.ICMT=(this.soundFontInfo.ICMT||"(No description)")+`
Converted from DLS to SF2 with SpessaSynth`,this.soundFontInfo.ISBJ&&(this.soundFontInfo.ICMT+=`
`+this.soundFontInfo.ISBJ,delete this.soundFontInfo.ISBJ);for(let[h,u]of Object.entries(this.soundFontInfo))x(`%c"${h}": %c"${u}"`,l.info,l.recognized);let a=t.find(h=>h.header==="colh");a||(v(),this.parsingError("No colh chunk!")),this.instrumentAmount=d(a.chunkData,4),x(`%cInstruments amount: %c${this.instrumentAmount}`,l.info,l.recognized);let B=aA(t,"wvpl");B||(v(),this.parsingError("No wvpl chunk!")),this.readDLSSamples(B);let C=aA(t,"lins");C||(v(),this.parsingError("No lins chunk!")),this.readDLSInstrumentList(C),this.presets.sort((h,u)=>h.program-u.program+(h.bank-u.bank)),x(`%cParsing finished! %c"${this.soundFontInfo.INAM||"UNNAMED"}"%c has %c${this.presets.length} %cpresets,
        %c${this.instruments.length}%c instruments and %c${this.samples.length}%c samples.`,l.info,l.recognized,l.info,l.recognized,l.info,l.recognized,l.info,l.recognized,l.info),v()}verifyHeader(A,e){A.header.toLowerCase()!==e.toLowerCase()&&(v(),this.parsingError(`Invalid DLS chunk header! Expected "${e.toLowerCase()}" got "${A.header.toLowerCase()}"`))}verifyText(A,e){A.toLowerCase()!==e.toLowerCase()&&(v(),this.parsingError(`FourCC error: Expected "${e.toLowerCase()}" got "${A.toLowerCase()}"`))}parsingError(A){throw new Error(`DLS parse error: ${A} The file may be corrupted.`)}};wA.prototype.readDLSInstrumentList=yt;wA.prototype.readDLSInstrument=wt;wA.prototype.readRegion=Gt;wA.prototype.readLart=Rt;wA.prototype.readDLSSamples=Mt;var MA=MA!==void 0?MA:{},Nt=!1,Ut;MA.isInitialized=new Promise(o=>Ut=o);var aI=function(o){var A,e,t,I,a,B,C,h="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",u="",S=0;o=o.replace(/[^A-Za-z0-9\+\/\=]/g,"");do I=h.indexOf(o.charAt(S++)),a=h.indexOf(o.charAt(S++)),B=h.indexOf(o.charAt(S++)),C=h.indexOf(o.charAt(S++)),A=I<<2|a>>4,e=(15&a)<<4|B>>2,t=(3&B)<<6|C,u+=String.fromCharCode(A),B!==64&&(u+=String.fromCharCode(e)),C!==64&&(u+=String.fromCharCode(t));while(S<o.length);return u};(function(){var o,A,e,t,I,a,B,C,h,u,S,f,Q,c,N,Y,W,M,J,T,O,q,g=g!==void 0?g:{};g.wasmBinary=Uint8Array.from(aI(""),function(n){return n.charCodeAt(0)});var g=g!==void 0?g:{},IA={};for(o in g)g.hasOwnProperty(o)&&(IA[o]=g[o]);g.arguments=[],g.thisProgram="./this.program",g.quit=function(n,s){throw s},g.preRun=[],g.postRun=[];var $=!1,mA=!1,j=!1,oA=!1;$=typeof window=="object",mA=typeof importScripts=="function",j=typeof process=="object"&&typeof Bt=="function"&&!$&&!mA,oA=!$&&!j&&!mA;var BA="";function ZA(n){return g.locateFile?g.locateFile(n,BA):BA+n}j?(BA=__dirname+"/",g.read=function(s,i){var E;return A||(A=void 0),e||(e=void 0),s=e.normalize(s),E=A.readFileSync(s),i?E:E.toString()},g.readBinary=function(s){var i=g.read(s,!0);return i.buffer||(i=new Uint8Array(i)),kA(i.buffer),i},process.argv.length>1&&(g.thisProgram=process.argv[1].replace(/\\/g,"/")),g.arguments=process.argv.slice(2),typeof module<"u",process.on("uncaughtException",function(n){if(!(n instanceof XA))throw n}),process.on("unhandledRejection",function(n,s){process.exit(1)}),g.quit=function(n){process.exit(n)},g.inspect=function(){return"[Emscripten Module object]"}):oA?(typeof read<"u"&&(g.read=function(s){return read(s)}),g.readBinary=function(s){var i;return typeof readbuffer=="function"?new Uint8Array(readbuffer(s)):(kA(typeof(i=read(s,"binary"))=="object"),i)},typeof scriptArgs<"u"?g.arguments=scriptArgs:typeof arguments<"u"&&(g.arguments=arguments),typeof quit=="function"&&(g.quit=function(n){quit(n)})):($||mA)&&($?document.currentScript&&(BA=document.currentScript.src):BA=self.location.href,BA=BA.indexOf("blob:")!==0?BA.split("/").slice(0,-1).join("/")+"/":"",g.read=function(s){var i=new XMLHttpRequest;return i.open("GET",s,!1),i.send(null),i.responseText},mA&&(g.readBinary=function(s){var i=new XMLHttpRequest;return i.open("GET",s,!1),i.responseType="arraybuffer",i.send(null),new Uint8Array(i.response)}),g.readAsync=function(s,i,E){var m=new XMLHttpRequest;m.open("GET",s,!0),m.responseType="arraybuffer",m.onload=function(){if(m.status==200||m.status==0&&m.response){i(m.response);return}E()},m.onerror=E,m.send(null)},g.setWindowTitle=function(n){document.title=n});var jA=g.print||(typeof console<"u"?console.log.bind(console):typeof print<"u"?print:null),CA=g.printErr||(typeof printErr<"u"?printErr:typeof console<"u"&&console.warn.bind(console)||jA);for(o in IA)IA.hasOwnProperty(o)&&(g[o]=IA[o]);function $A(n){var s=c;return c=c+n+15&-16,s}function Ve(n){var s=h[T>>2],i=s+n+15&-16;return h[T>>2]=i,i>=eA&&!tt()?(h[T>>2]=s,0):s}function Pe(n,s){return s||(s=16),n=Math.ceil(n/s)*s}function Zt(n){switch(n){case"i1":case"i8":return 1;case"i16":return 2;case"i32":case"float":return 4;case"i64":case"double":return 8;default:if(n[n.length-1]==="*")return 4;if(n[0]!=="i")return 0;var s=parseInt(n.substr(1));return kA(s%8==0),s/8}}function OA(n){OA.shown||(OA.shown={}),OA.shown[n]||(OA.shown[n]=1,CA(n))}IA=void 0;var Ot={"f64-rem":function(n,s){return n%s},debugger:function(){}},Qe=[];function mI(n,s){for(var i=0,E=i;E<i+0;E++)if(!Qe[E])return Qe[E]=n,1+E;throw"Finished up all reserved function pointers. Use a higher value for RESERVED_FUNCTION_POINTERS."}function pI(n){Qe[n-1]=null}var he={};function DI(n,s){if(n){kA(s),he[s]||(he[s]={});var i=he[s];return i[n]||(s.length===1?i[n]=function(){return ce(s,n)}:s.length===2?i[n]=function(m){return ce(s,n,[m])}:i[n]=function(){return ce(s,n,Array.prototype.slice.call(arguments))}),i[n]}}function SI(n,s,i){return i?+(n>>>0)+4294967296*+(s>>>0):+(n>>>0)+4294967296*+(0|s)}function ce(n,s,i){return i&&i.length?g["dynCall_"+n].apply(null,[s].concat(i)):g["dynCall_"+n].call(null,s)}var Ae=0,Xe=0;function kA(n,s){n||SA("Assertion failed: "+s)}function We(n){var s=g["_"+n];return kA(s,"Cannot call unknown function "+n+", make sure it is exported"),s}var _e={stackSave:function(){Se()},stackRestore:function(){De()},arrayToC:function(n){var s,i,E=ge(n.length);return s=n,i=E,I.set(s,i),E},stringToC:function(n){var s=0;if(n!=null&&n!==0){var i=(n.length<<2)+1;s=ge(i),$e(n,s,i)}return s}},Vt={string:_e.stringToC,array:_e.arrayToC};function ze(n,s,i,E,m){var G=We(n),U=[],D=0;if(E)for(var tA=0;tA<E.length;tA++){var _=Vt[i[tA]];_?(D===0&&(D=Se()),U[tA]=_(E[tA])):U[tA]=E[tA]}var z,P=G.apply(null,U);return P=(z=P,s==="string"?ee(z):s==="boolean"?!!z:z),D!==0&&De(D),P}function Pt(n,s,i,E){switch((i=i||"i8").charAt(i.length-1)==="*"&&(i="i32"),i){case"i1":case"i8":I[n>>0]=s;break;case"i16":B[n>>1]=s;break;case"i32":h[n>>2]=s;break;case"i64":tempI64=[s>>>0,+jt(tempDouble=s)>=1?tempDouble>0?(0|AI(+ot(tempDouble/4294967296),4294967295))>>>0:~~+$t((tempDouble-+(~~tempDouble>>>0))/4294967296)>>>0:0],h[n>>2]=tempI64[0],h[n+4>>2]=tempI64[1];break;case"float":S[n>>2]=s;break;case"double":f[n>>3]=s;break;default:SA("invalid type for setValue: "+i)}}function yI(n,s,i){switch((s=s||"i8").charAt(s.length-1)==="*"&&(s="i32"),s){case"i1":case"i8":return I[n>>0];case"i16":return B[n>>1];case"i32":case"i64":return h[n>>2];case"float":return S[n>>2];case"double":return f[n>>3];default:SA("invalid type for getValue: "+s)}return null}function wI(n,s,i,E){typeof n=="number"?(G=!0,U=n):(G=!1,U=n.length);var m=typeof s=="string"?s:null;if(D=i==4?E:[typeof se=="function"?se:$A,ge,$A,Ve][i===void 0?2:i](Math.max(U,m?1:s.length)),G){for(E=D,kA((3&D)==0),tA=D+(-4&U);E<tA;E+=4)h[E>>2]=0;for(tA=D+U;E<tA;)I[E++>>0]=0;return D}if(m==="i8")return n.subarray||n.slice?a.set(n,D):a.set(new Uint8Array(n),D),D;for(var G,U,D,tA,_,z,P,R=0;R<U;){var iA=n[R];if((_=m||s[R])===0){R++;continue}_=="i64"&&(_="i32"),Pt(D+R,iA,_),P!==_&&(z=Zt(_),P=_),R+=z}return D}function FI(n){return N?me?se(n):Ve(n):$A(n)}function ee(n,s){if(s===0||!n)return"";for(var i,E,m,G=0,U=0;G|=E=a[n+U>>0],(E!=0||s)&&(U++,!s||U!=s););s||(s=U);var D="";if(G<128){for(;s>0;)m=String.fromCharCode.apply(String,a.subarray(n,n+Math.min(s,1024))),D=D?D+m:m,n+=1024,s-=1024;return D}return i=n,function(_,z){for(var P=z;_[P];)++P;if(P-z>16&&_.subarray&&je)return je.decode(_.subarray(z,P));for(var R,iA,QA,hA,cA,LA,lA="";;){if(!(R=_[z++]))return lA;if(!(128&R)){lA+=String.fromCharCode(R);continue}if(iA=63&_[z++],(224&R)==192){lA+=String.fromCharCode((31&R)<<6|iA);continue}if(QA=63&_[z++],(240&R)==224?R=(15&R)<<12|iA<<6|QA:(hA=63&_[z++],(248&R)==240?R=(7&R)<<18|iA<<12|QA<<6|hA:(cA=63&_[z++],R=(252&R)==248?(3&R)<<24|iA<<18|QA<<12|hA<<6|cA:(1&R)<<30|iA<<24|QA<<18|hA<<12|cA<<6|(LA=63&_[z++]))),R<65536)lA+=String.fromCharCode(R);else{var WA=R-65536;lA+=String.fromCharCode(55296|WA>>10,56320|1023&WA)}}}(a,i)}function kI(n){for(var s="";;){var i=I[n++>>0];if(!i)return s;s+=String.fromCharCode(i)}}function RI(n,s){return function(E,m,G){for(var U=0;U<E.length;++U)I[m++>>0]=E.charCodeAt(U);G||(I[m>>0]=0)}(n,s,!1)}var je=typeof TextDecoder<"u"?new TextDecoder("utf8"):void 0;function te(n,s,i,E){if(!(E>0))return 0;for(var m=i,G=i+E-1,U=0;U<n.length;++U){var D=n.charCodeAt(U);if(D>=55296&&D<=57343&&(D=65536+((1023&D)<<10)|1023&n.charCodeAt(++U)),D<=127){if(i>=G)break;s[i++]=D}else if(D<=2047){if(i+1>=G)break;s[i++]=192|D>>6,s[i++]=128|63&D}else if(D<=65535){if(i+2>=G)break;s[i++]=224|D>>12,s[i++]=128|D>>6&63,s[i++]=128|63&D}else if(D<=2097151){if(i+3>=G)break;s[i++]=240|D>>18,s[i++]=128|D>>12&63,s[i++]=128|D>>6&63,s[i++]=128|63&D}else if(D<=67108863){if(i+4>=G)break;s[i++]=248|D>>24,s[i++]=128|D>>18&63,s[i++]=128|D>>12&63,s[i++]=128|D>>6&63,s[i++]=128|63&D}else{if(i+5>=G)break;s[i++]=252|D>>30,s[i++]=128|D>>24&63,s[i++]=128|D>>18&63,s[i++]=128|D>>12&63,s[i++]=128|D>>6&63,s[i++]=128|63&D}}return s[i]=0,i-m}function $e(n,s,i){return te(n,a,s,i)}function Ie(n){for(var s=0,i=0;i<n.length;++i){var E=n.charCodeAt(i);E>=55296&&E<=57343&&(E=65536+((1023&E)<<10)|1023&n.charCodeAt(++i)),E<=127?++s:E<=2047?s+=2:E<=65535?s+=3:E<=2097151?s+=4:E<=67108863?s+=5:s+=6}return s}var At=typeof TextDecoder<"u"?new TextDecoder("utf-16le"):void 0;function GI(n){for(var s=n,i=s>>1;B[i];)++i;if((s=i<<1)-n>32&&At)return At.decode(a.subarray(n,s));for(var E=0,m="";;){var G=B[n+2*E>>1];if(G==0)return m;++E,m+=String.fromCharCode(G)}}function xI(n,s,i){if(i===void 0&&(i=2147483647),i<2)return 0;for(var E=s,m=(i-=2)<2*n.length?i/2:n.length,G=0;G<m;++G){var U=n.charCodeAt(G);B[s>>1]=U,s+=2}return B[s>>1]=0,s-E}function MI(n){return 2*n.length}function NI(n){for(var s=0,i="";;){var E=h[n+4*s>>2];if(E==0)return i;if(++s,E>=65536){var m=E-65536;i+=String.fromCharCode(55296|m>>10,56320|1023&m)}else i+=String.fromCharCode(E)}}function UI(n,s,i){if(i===void 0&&(i=2147483647),i<4)return 0;for(var E=s,m=E+i-4,G=0;G<n.length;++G){var U=n.charCodeAt(G);if(U>=55296&&U<=57343&&(U=65536+((1023&U)<<10)|1023&n.charCodeAt(++G)),h[s>>2]=U,(s+=4)+4>m)break}return h[s>>2]=0,s-E}function LI(n){for(var s=0,i=0;i<n.length;++i){var E=n.charCodeAt(i);E>=55296&&E<=57343&&++i,s+=4}return s}function bI(n){var s=Ie(n)+1,i=se(s);return i&&te(n,I,i,s),i}function HI(n){var s=Ie(n)+1,i=ge(s);return te(n,I,i,s),i}function YI(n){return n}function JI(){var n,s=function(){var E=Error();if(!E.stack){try{throw Error(0)}catch(m){E=m}if(!E.stack)return"(no stack trace available)"}return E.stack.toString()}();return g.extraStackTrace&&(s+=`
`+g.extraStackTrace()),(n=s).replace(/__Z[\w\d_]+/g,function(i){var E,m=E=i;return i===m?i:i+" ["+m+"]"})}function le(n,s){return n%s>0&&(n+=s-n%s),n}function et(n){g.buffer=t=n}function de(){g.HEAP8=I=new Int8Array(t),g.HEAP16=B=new Int16Array(t),g.HEAP32=h=new Int32Array(t),g.HEAPU8=a=new Uint8Array(t),g.HEAPU16=C=new Uint16Array(t),g.HEAPU32=u=new Uint32Array(t),g.HEAPF32=S=new Float32Array(t),g.HEAPF64=f=new Float64Array(t)}function tt(){var n=g.usingWasm?65536:16777216,s=2147483648-n;if(h[T>>2]>s)return!1;var i=eA;for(eA=Math.max(eA,16777216);eA<h[T>>2];)eA=eA<=536870912?le(2*eA,n):Math.min(le((3*eA+2147483648)/4,n),s);var E=g.reallocBuffer(eA);return E&&E.byteLength==eA?(et(E),de(),!0):(eA=i,!1)}Q=c=Y=W=M=J=T=0,N=!1,g.reallocBuffer||(g.reallocBuffer=function(n){try{if(ArrayBuffer.transfer)s=ArrayBuffer.transfer(t,n);else{var s,i=I;s=new ArrayBuffer(n),new Int8Array(s).set(i)}}catch{return!1}return!!eI(s)&&s});try{(O=Function.prototype.call.bind(Object.getOwnPropertyDescriptor(ArrayBuffer.prototype,"byteLength").get))(new ArrayBuffer(4))}catch{O=function(s){return s.byteLength}}var fe=g.TOTAL_STACK||5242880,eA=g.TOTAL_MEMORY||16777216;function Xt(){return eA}function VA(n){for(;n.length>0;){var s=n.shift();if(typeof s=="function"){s();continue}var i=s.func;typeof i=="number"?s.arg===void 0?g.dynCall_v(i):g.dynCall_vi(i,s.arg):i(s.arg===void 0?null:s.arg)}}eA<fe&&CA("TOTAL_MEMORY should be larger than TOTAL_STACK, was "+eA+"! (TOTAL_STACK="+fe+")"),g.buffer?t=g.buffer:(typeof WebAssembly=="object"&&typeof WebAssembly.Memory=="function"?(g.wasmMemory=new WebAssembly.Memory({initial:eA/65536}),t=g.wasmMemory.buffer):t=new ArrayBuffer(eA),g.buffer=t),de();var It=[],ue=[],nt=[],st=[],gt=[],me=!1,Wt=!1;function _t(n){It.unshift(n)}function qI(n){ue.unshift(n)}function TI(n){nt.unshift(n)}function vI(n){st.unshift(n)}function zt(n){gt.unshift(n)}function KI(n,s,i){var E,m;OA("writeStringToMemory is deprecated and should not be called! Use stringToUTF8() instead!"),i&&(E=I[m=s+Ie(n)]),$e(n,s,1/0),i&&(I[m]=E)}function ZI(n,s,i){return n>=0?n:s<=32?2*Math.abs(1<<s-1)+n:Math.pow(2,s)+n}function OI(n,s,i){if(n<=0)return n;var E=s<=32?Math.abs(1<<s-1):Math.pow(2,s-1);return n>=E&&(s<=32||n>E)&&(n=-2*E+n),n}var jt=Math.abs,$t=Math.ceil,ot=Math.floor,AI=Math.min,UA=0,pe=null,PA=null;function VI(n){return n}g.preloadedImages={},g.preloadedAudios={};var it="data:application/octet-stream;base64,";function ne(n){return String.prototype.startsWith?n.startsWith(it):n.indexOf(it)===0}(function(){var s="main.wast",i="main.wasm",E="main.temp.asm.js";ne(s)||(s=ZA(s)),ne(i)||(i=ZA(i)),ne(E)||(E=ZA(E));var m={global:null,env:null,asm2wasm:Ot,parent:g},G=null;function U(P){return P}function D(){try{if(g.wasmBinary)return new Uint8Array(g.wasmBinary);if(g.readBinary)return g.readBinary(i);throw"both async and sync fetching of the wasm failed"}catch(P){SA(P)}}g.asmPreload=g.asm;var tA=g.reallocBuffer,_=function(P){P=le(P,g.usingWasm?65536:16777216);var R=g.buffer.byteLength;if(g.usingWasm)try{var iA=g.wasmMemory.grow((P-R)/65536);return iA!==-1?g.buffer=g.wasmMemory.buffer:null}catch{return null}};g.reallocBuffer=function(P){return z==="asmjs"?tA(P):_(P)};var z="";g.asm=function(P,R,iA){var QA;if(!(R=QA=R).table){var hA,cA=g.wasmTableSize;cA===void 0&&(cA=1024);var LA=g.wasmMaxTableSize;typeof WebAssembly=="object"&&typeof WebAssembly.Table=="function"?LA!==void 0?R.table=new WebAssembly.Table({initial:cA,maximum:LA,element:"anyfunc"}):R.table=new WebAssembly.Table({initial:cA,element:"anyfunc"}):R.table=Array(cA),g.wasmTable=R.table}return R.memoryBase||(R.memoryBase=g.STATIC_BASE),R.tableBase||(R.tableBase=0),hA=function(WA,HA,we){if(typeof WebAssembly!="object")return CA("no native wasm support detected"),!1;if(!(g.wasmMemory instanceof WebAssembly.Memory))return CA("no native wasm Memory in use"),!1;function oe(pA,DA){if((G=pA.exports).memory){var _A,Fe,Et;_A=G.memory,Fe=g.buffer,_A.byteLength<Fe.byteLength&&CA("the new buffer in mergeMemory is smaller than the previous one. in native wasm, we should grow memory here"),Et=new Int8Array(Fe),new Int8Array(_A).set(Et),et(_A),de()}g.asm=G,g.usingWasm=!0,function($I){if(UA--,g.monitorRunDependencies&&g.monitorRunDependencies(UA),UA==0&&(pe!==null&&(clearInterval(pe),pe=null),PA)){var tI=PA;PA=null,tI()}}("wasm-instantiate")}if(HA.memory=g.wasmMemory,m.global={NaN:NaN,Infinity:1/0},m["global.Math"]=Math,m.env=HA,UA++,g.monitorRunDependencies&&g.monitorRunDependencies(UA),g.instantiateWasm)try{return g.instantiateWasm(m,oe)}catch(pA){return CA("Module.instantiateWasm callback failed with error: "+pA),!1}function YA(pA){oe(pA.instance,pA.module)}function at(pA){(!g.wasmBinary&&($||mA)&&typeof fetch=="function"?fetch(i,{credentials:"same-origin"}).then(function(DA){if(!DA.ok)throw"failed to load wasm binary file at '"+i+"'";return DA.arrayBuffer()}).catch(function(){return D()}):new Promise(function(DA,_A){DA(D())})).then(function(DA){return WebAssembly.instantiate(DA,m)}).then(pA).catch(function(DA){CA("failed to asynchronously prepare wasm: "+DA),SA(DA)})}return g.wasmBinary||typeof WebAssembly.instantiateStreaming!="function"||ne(i)||typeof fetch!="function"?at(YA):WebAssembly.instantiateStreaming(fetch(i,{credentials:"same-origin"}),m).then(YA).catch(function(pA){CA("wasm streaming compile failed: "+pA),CA("falling back to ArrayBuffer instantiation"),at(YA)}),{}}(P,R,iA),kA(hA,"no binaryen method succeeded."),hA},g.asm})(),c=(Q=1024)+4816,ue.push(),g.STATIC_BASE=Q,g.STATIC_BUMP=4816;var EA=c;function PI(n){I[EA]=I[n],I[EA+1]=I[n+1],I[EA+2]=I[n+2],I[EA+3]=I[n+3]}function XI(n){I[EA]=I[n],I[EA+1]=I[n+1],I[EA+2]=I[n+2],I[EA+3]=I[n+3],I[EA+4]=I[n+4],I[EA+5]=I[n+5],I[EA+6]=I[n+6],I[EA+7]=I[n+7]}function WI(n,s,i){var E=i>0?i:Ie(n)+1,m=Array(E),G=te(n,m,0,m.length);return s&&(m.length=G),m}function _I(n){for(var s=[],i=0;i<n.length;i++){var E=n[i];E>255&&(E&=255),s.push(String.fromCharCode(E))}return s.join("")}c+=16,T=$A(4),M=(Y=W=Pe(c))+fe,J=Pe(M),h[T>>2]=J,N=!0,g.wasmTableSize=4,g.wasmMaxTableSize=4,g.asmGlobalArg={},g.asmLibraryArg={abort:SA,assert:kA,enlargeMemory:tt,getTotalMemory:Xt,abortOnCannotGrowMemory:function(){SA("Cannot enlarge memory arrays. Either (1) compile with  -s TOTAL_MEMORY=X  with X higher than the current value "+eA+", (2) compile with  -s ALLOW_MEMORY_GROWTH=1  which allows increasing the size at runtime, or (3) if you want malloc to return NULL (0) instead of this abort, compile with  -s ABORTING_MALLOC=0 ")},invoke_iii:function(s,i,E){var m=Se();try{return g.dynCall_iii(s,i,E)}catch(G){if(De(m),typeof G!="number"&&G!=="longjmp")throw G;g.setThrew(1,0)}},___assert_fail:function(s,i,E,m){SA("Assertion failed: "+ee(s)+", at: "+[i?ee(i):"unknown filename",E,m?ee(m):"unknown function"])},___setErrNo:function(s){return g.___errno_location&&(h[g.___errno_location()>>2]=s),s},_abort:function(){g.abort()},_emscripten_memcpy_big:function(s,i,E){return a.set(a.subarray(i,i+E),s),s},_llvm_floor_f64:ot,DYNAMICTOP_PTR:T,tempDoublePtr:EA,ABORT:Ae,STACKTOP:W,STACK_MAX:M};var rt=g.asm(g.asmGlobalArg,g.asmLibraryArg,t);g.asm=rt,g.___errno_location=function(){return g.asm.___errno_location.apply(null,arguments)};var eI=g._emscripten_replace_memory=function(){return g.asm._emscripten_replace_memory.apply(null,arguments)};g._free=function(){return g.asm._free.apply(null,arguments)};var se=g._malloc=function(){return g.asm._malloc.apply(null,arguments)};g._memcpy=function(){return g.asm._memcpy.apply(null,arguments)},g._memset=function(){return g.asm._memset.apply(null,arguments)},g._sbrk=function(){return g.asm._sbrk.apply(null,arguments)},g._stb_vorbis_js_channels=function(){return g.asm._stb_vorbis_js_channels.apply(null,arguments)},g._stb_vorbis_js_close=function(){return g.asm._stb_vorbis_js_close.apply(null,arguments)},g._stb_vorbis_js_decode=function(){return g.asm._stb_vorbis_js_decode.apply(null,arguments)},g._stb_vorbis_js_open=function(){return g.asm._stb_vorbis_js_open.apply(null,arguments)},g._stb_vorbis_js_sample_rate=function(){return g.asm._stb_vorbis_js_sample_rate.apply(null,arguments)},g.establishStackSpace=function(){return g.asm.establishStackSpace.apply(null,arguments)},g.getTempRet0=function(){return g.asm.getTempRet0.apply(null,arguments)},g.runPostSets=function(){return g.asm.runPostSets.apply(null,arguments)},g.setTempRet0=function(){return g.asm.setTempRet0.apply(null,arguments)},g.setThrew=function(){return g.asm.setThrew.apply(null,arguments)};var ge=g.stackAlloc=function(){return g.asm.stackAlloc.apply(null,arguments)},De=g.stackRestore=function(){return g.asm.stackRestore.apply(null,arguments)},Se=g.stackSave=function(){return g.asm.stackSave.apply(null,arguments)};function XA(n){this.name="ExitStatus",this.message="Program terminated with exit("+n+")",this.status=n}function ye(n){n=n||g.arguments,!(UA>0)&&(function(){if(g.preRun)for(typeof g.preRun=="function"&&(g.preRun=[g.preRun]);g.preRun.length;)_t(g.preRun.shift());VA(It)}(),!(UA>0)&&(g.calledRun||(g.setStatus?(g.setStatus("Running..."),setTimeout(function(){setTimeout(function(){g.setStatus("")},1),s()},1)):s())));function s(){!g.calledRun&&(g.calledRun=!0,Ae||(me||(me=!0,VA(ue)),VA(nt),g.onRuntimeInitialized&&g.onRuntimeInitialized(),function(){if(g.postRun)for(typeof g.postRun=="function"&&(g.postRun=[g.postRun]);g.postRun.length;)zt(g.postRun.shift());VA(gt)}()))}}function zI(n,s){(!s||!g.noExitRuntime||n!==0)&&(g.noExitRuntime||(Ae=!0,Xe=n,W=q,VA(st),Wt=!0,g.onExit&&g.onExit(n)),g.quit(n,new XA(n)))}function SA(n){throw g.onAbort&&g.onAbort(n),n!==void 0?(jA(n),CA(n),n=JSON.stringify(n)):n="",Ae=!0,Xe=1,"abort("+n+"). Build with -s ASSERTIONS=1 for more info."}if(g.dynCall_iii=function(){return g.asm.dynCall_iii.apply(null,arguments)},g.asm=rt,g.ccall=ze,g.cwrap=function(s,i,E,m){var G=(E=E||[]).every(function(U){return U==="number"});return i!=="string"&&G&&!m?We(s):function(){return ze(s,i,E,arguments,m)}},XA.prototype=Error(),XA.prototype.constructor=XA,PA=function n(){g.calledRun||ye(),g.calledRun||(PA=n)},g.run=ye,g.abort=SA,g.preInit)for(typeof g.preInit=="function"&&(g.preInit=[g.preInit]);g.preInit.length>0;)g.preInit.pop()();g.noExitRuntime=!0,ye(),g.onRuntimeInitialized=()=>{Nt=!0,Ut()},MA.decode=function(n){return function(i){if(!Nt)throw Error("Not initialized");var E={};function m(HA){return new Int32Array(g.HEAPU8.buffer,HA,1)[0]}function G(HA,we){var oe=new ArrayBuffer(we*Float32Array.BYTES_PER_ELEMENT),YA=new Float32Array(oe);return YA.set(new Float32Array(g.HEAPU8.buffer,HA,we)),YA}E.open=g.cwrap("stb_vorbis_js_open","number",[]),E.close=g.cwrap("stb_vorbis_js_close","void",["number"]),E.channels=g.cwrap("stb_vorbis_js_channels","number",["number"]),E.sampleRate=g.cwrap("stb_vorbis_js_sample_rate","number",["number"]),E.decode=g.cwrap("stb_vorbis_js_decode","number",["number","number","number","number","number"]);var U,D,tA,_,z=E.open(),P=(U=i,D=i.byteLength,tA=g._malloc(D),(_=new Uint8Array(g.HEAPU8.buffer,tA,D)).set(new Uint8Array(U,0,D)),_),R=g._malloc(4),iA=g._malloc(4),QA=E.decode(z,P.byteOffset,P.byteLength,R,iA);if(g._free(P.byteOffset),QA<0)throw E.close(z),g._free(R),Error("stbvorbis decode failed: "+QA);for(var hA=E.channels(z),cA=Array(hA),LA=new Int32Array(g.HEAPU32.buffer,m(R),hA),lA=0;lA<hA;lA++)cA[lA]=G(LA[lA],QA),g._free(LA[lA]);var WA=E.sampleRate(z);return E.close(z),g._free(m(R)),g._free(R),{data:cA,sampleRate:WA,eof:!0,error:null}}(n)}})();var Me=class extends KA{constructor(A,e,t,I,a,B,C,h,u,S,f,Q,c){super(A,B,C,h,u,S,I-e/2,a-e/2),this.sampleName=A,this.sampleStartIndex=e,this.sampleEndIndex=t,this.isSampleLoaded=!1,this.sampleID=Q,this.sampleLength=this.sampleEndIndex-this.sampleStartIndex,this.sampleDataArray=f,this.sampleData=new Float32Array(0),this.isCompressed&&(this.sampleLoopStartIndex+=this.sampleStartIndex/2,this.sampleLoopEndIndex+=this.sampleStartIndex/2,this.sampleLength=99999999),this.isDataRaw=c}getRawData(){let A=this.sampleDataArray;if(this.isCompressed){if(this.compressedData)return this.compressedData;let e=A.currentIndex;return A.slice(this.sampleStartIndex/2+e,this.sampleEndIndex/2+e)}else{if(!this.isDataRaw)throw new Error("Writing SF2Pack samples is not supported.");let e=A.currentIndex;return A.slice(e+this.sampleStartIndex,e+this.sampleEndIndex)}}decodeVorbis(){if(this.sampleLength<1)return;let A=this.sampleDataArray,e=A.currentIndex,t=A.slice(this.sampleStartIndex/2+e,this.sampleEndIndex/2+e);this.sampleData=new Float32Array(0);try{let I=MA.decode(t.buffer);this.sampleData=I.data[0]}catch(I){throw new Error(`Ogg Vorbis decode error: ${I}`)}}getAudioData(){return this.isSampleLoaded?this.sampleData:this.sampleLength<1?new Float32Array(1):this.isCompressed?(this.decodeVorbis(),this.isSampleLoaded=!0,this.sampleData):this.isDataRaw?this.loadUncompressedData():this.getUncompressedReadyData()}loadUncompressedData(){if(this.isCompressed)return AA("Trying to load a compressed sample via loadUncompressedData()... aborting!"),new Float32Array(0);let A=new Float32Array(this.sampleLength/2),e=this.sampleDataArray.currentIndex,t=new Int16Array(this.sampleDataArray.slice(e+this.sampleStartIndex,e+this.sampleEndIndex).buffer);for(let I=0;I<t.length;I++)A[I]=t[I]/32768;return this.sampleData=A,this.isSampleLoaded=!0,A}getUncompressedReadyData(){let A=this.sampleDataArray.slice(this.sampleStartIndex/2,this.sampleEndIndex/2);return this.sampleData=A,this.isSampleLoaded=!0,A}};function Lt(o,A,e=!0){let t=[],I=0;for(;o.chunkData.length>o.chunkData.currentIndex;){let a=EI(I,o.chunkData,A,e);t.push(a),I++}return t.length>1&&t.pop(),t}function EI(o,A,e,t){let I=V(A,20),a=d(A,4)*2,B=d(A,4)*2,C=d(A,4),h=d(A,4),u=d(A,4),S=A[A.currentIndex++];S===255&&(S=60);let f=Ct(A[A.currentIndex++]),Q=d(A,2),c=d(A,2);return new Me(I,a,B,C,h,u,S,f,Q,c,e,o,t)}var Ne=class extends y{constructor(A){super();let e=A.currentIndex;this.generatorType=A[e+1]<<8|A[e],this.generatorValue=RA(A[e+2],A[e+3]),A.currentIndex+=4}};function Ue(o){let A=[];for(;o.chunkData.length>o.chunkData.currentIndex;)A.push(new Ne(o.chunkData));return A.length>1&&A.pop(),A}var Le=class extends vA{constructor(A){super(),this.instrumentName=V(A.chunkData,20).trim(),this.instrumentZoneIndex=d(A.chunkData,2),this.instrumentZonesAmount=0}getInstrumentZones(A,e){this.instrumentZonesAmount=A;for(let t=this.instrumentZoneIndex;t<this.instrumentZonesAmount+this.instrumentZoneIndex;t++)this.instrumentZones.push(e[t])}};function bt(o,A){let e=[];for(;o.chunkData.length>o.chunkData.currentIndex;){let t=new Le(o);if(e.length>0){let I=t.instrumentZoneIndex-e[e.length-1].instrumentZoneIndex;e[e.length-1].getInstrumentZones(I,A)}e.push(t)}return e.length>1&&e.pop(),e}var be=class extends xA{constructor(A){super(),this.generatorZoneStartIndex=d(A,2),this.modulatorZoneStartIndex=d(A,2),this.modulatorZoneSize=0,this.generatorZoneSize=0,this.isGlobal=!0}setZoneSize(A,e){this.modulatorZoneSize=A,this.generatorZoneSize=e}getGenerators(A){for(let e=this.generatorZoneStartIndex;e<this.generatorZoneStartIndex+this.generatorZoneSize;e++)this.generators.push(A[e])}getModulators(A){for(let e=this.modulatorZoneStartIndex;e<this.modulatorZoneStartIndex+this.modulatorZoneSize;e++)this.modulators.push(A[e])}getSample(A){let e=this.generators.find(t=>t.generatorType===r.sampleID);e&&(this.sample=A[e.generatorValue],this.isGlobal=!1,this.sample.useCount++)}getKeyRange(){let A=this.generators.find(e=>e.generatorType===r.keyRange);A&&(this.keyRange.min=A.generatorValue&127,this.keyRange.max=A.generatorValue>>8&127)}getVelRange(){let A=this.generators.find(e=>e.generatorType===r.velRange);A&&(this.velRange.min=A.generatorValue&127,this.velRange.max=A.generatorValue>>8&127)}};function Ht(o,A,e,t){let I=[];for(;o.chunkData.length>o.chunkData.currentIndex;){let a=new be(o.chunkData);if(I.length>0){let B=a.modulatorZoneStartIndex-I[I.length-1].modulatorZoneStartIndex,C=a.generatorZoneStartIndex-I[I.length-1].generatorZoneStartIndex;I[I.length-1].setZoneSize(B,C),I[I.length-1].getGenerators(A),I[I.length-1].getModulators(e),I[I.length-1].getSample(t),I[I.length-1].getKeyRange(),I[I.length-1].getVelRange()}I.push(a)}return I.length>1&&I.pop(),I}var He=class extends TA{constructor(A){super(),this.generatorZoneStartIndex=d(A,2),this.modulatorZoneStartIndex=d(A,2),this.modulatorZoneSize=0,this.generatorZoneSize=0,this.isGlobal=!0}setZoneSize(A,e){this.modulatorZoneSize=A,this.generatorZoneSize=e}getGenerators(A){for(let e=this.generatorZoneStartIndex;e<this.generatorZoneStartIndex+this.generatorZoneSize;e++)this.generators.push(A[e])}getModulators(A){for(let e=this.modulatorZoneStartIndex;e<this.modulatorZoneStartIndex+this.modulatorZoneSize;e++)this.modulators.push(A[e])}getInstrument(A){let e=this.generators.find(t=>t.generatorType===r.instrument);e&&(this.instrument=A[e.generatorValue],this.instrument.addUseCount(),this.isGlobal=!1)}getKeyRange(){let A=this.generators.find(e=>e.generatorType===r.keyRange);A&&(this.keyRange.min=A.generatorValue&127,this.keyRange.max=A.generatorValue>>8&127)}getVelRange(){let A=this.generators.find(e=>e.generatorType===r.velRange);A&&(this.velRange.min=A.generatorValue&127,this.velRange.max=A.generatorValue>>8&127)}};function Yt(o,A,e,t){let I=[];for(;o.chunkData.length>o.chunkData.currentIndex;){let a=new He(o.chunkData);if(I.length>0){let B=a.modulatorZoneStartIndex-I[I.length-1].modulatorZoneStartIndex,C=a.generatorZoneStartIndex-I[I.length-1].generatorZoneStartIndex;I[I.length-1].setZoneSize(B,C),I[I.length-1].getGenerators(A),I[I.length-1].getModulators(e),I[I.length-1].getInstrument(t),I[I.length-1].getKeyRange(),I[I.length-1].getVelRange()}I.push(a)}return I.length>1&&I.pop(),I}var Ye=class extends qA{constructor(A,e){super(e),this.presetName=V(A.chunkData,20).trim().replace(/\d{3}:\d{3}/,""),this.program=d(A.chunkData,2),this.bank=d(A.chunkData,2),this.presetZoneStartIndex=d(A.chunkData,2),this.library=d(A.chunkData,4),this.genre=d(A.chunkData,4),this.morphology=d(A.chunkData,4),this.presetZonesAmount=0}getPresetZones(A,e){this.presetZonesAmount=A;for(let t=this.presetZoneStartIndex;t<this.presetZonesAmount+this.presetZoneStartIndex;t++)this.presetZones.push(e[t])}};function Jt(o,A,e){let t=[];for(;o.chunkData.length>o.chunkData.currentIndex;){let I=new Ye(o,e);if(t.length>0){let a=I.presetZoneStartIndex-t[t.length-1].presetZoneStartIndex;t[t.length-1].getPresetZones(a,A)}t.push(I)}return t.length>1&&t.pop(),t}var Je=class extends K{constructor(A){super({srcEnum:d(A,2),dest:d(A,2),amt:RA(A[A.currentIndex++],A[A.currentIndex++]),secSrcEnum:d(A,2),transform:d(A,2)})}};function Be(o){let A=[];for(;o.chunkData.length>o.chunkData.currentIndex;)A.push(new Je(o.chunkData));return A}var Ce=class extends GA{constructor(A,e=!0){super(),e&&console.warn("Using the constructor directly is deprecated. Use loadSoundFont instead."),this.dataArray=new F(A),yA("%cParsing SoundFont...",l.info),this.dataArray||(v(),this.parsingError("No data provided!"));let t=L(this.dataArray,!1);this.verifyHeader(t,"riff");let I=V(this.dataArray,4).toLowerCase();if(I!=="sfbk"&&I!=="sfpk")throw v(),new SyntaxError(`Invalid soundFont! Expected "sfbk" or "sfpk" got "${I}"`);let a=I==="sfpk",B=L(this.dataArray);for(this.verifyHeader(B,"list"),V(B.chunkData,4);B.chunkData.length>B.chunkData.currentIndex;){let j=L(B.chunkData),oA;switch(j.header.toLowerCase()){case"ifil":case"iver":oA=`${d(j.chunkData,2)}.${d(j.chunkData,2)}`,this.soundFontInfo[j.header]=oA;break;case"icmt":oA=V(j.chunkData,j.chunkData.length,void 0,!1),this.soundFontInfo[j.header]=oA;break;case"dmod":let BA=Be(j);BA.pop(),oA=`Modulators: ${BA.length}`;let ZA=this.defaultModulators;this.defaultModulators=BA,this.defaultModulators.push(...ZA.filter(jA=>!this.defaultModulators.find(CA=>K.isIdentical(jA,CA)))),this.soundFontInfo[j.header]=j.chunkData;break;default:oA=V(j.chunkData,j.chunkData.length),this.soundFontInfo[j.header]=oA}x(`%c"${j.header}": %c"${oA}"`,l.info,l.recognized)}let C=L(this.dataArray,!1);this.verifyHeader(C,"list"),this.verifyText(V(this.dataArray,4),"sdta"),x("%cVerifying smpl chunk...",l.warn);let h=L(this.dataArray,!1);this.verifyHeader(h,"smpl");let u;if(a){x("%cSF2Pack detected, attempting to decode the smpl chunk...",l.info);try{u=MA.decode(this.dataArray.buffer.slice(this.dataArray.currentIndex,this.dataArray.currentIndex+C.size-12)).data[0]}catch(j){throw v(),new Error(`SF2Pack Ogg Vorbis decode error: ${j}`)}x(`%cDecoded the smpl chunk! Length: %c${u.length}`,l.info,l.value)}else u=this.dataArray,this.sampleDataStartIndex=this.dataArray.currentIndex;x(`%cSkipping sample chunk, length: %c${C.size-12}`,l.info,l.value),this.dataArray.currentIndex+=C.size-12,x("%cLoading preset data chunk...",l.warn);let S=L(this.dataArray);this.verifyHeader(S,"list"),V(S.chunkData,4);let f=L(S.chunkData);this.verifyHeader(f,"phdr");let Q=L(S.chunkData);this.verifyHeader(Q,"pbag");let c=L(S.chunkData);this.verifyHeader(c,"pmod");let N=L(S.chunkData);this.verifyHeader(N,"pgen");let Y=L(S.chunkData);this.verifyHeader(Y,"inst");let W=L(S.chunkData);this.verifyHeader(W,"ibag");let M=L(S.chunkData);this.verifyHeader(M,"imod");let J=L(S.chunkData);this.verifyHeader(J,"igen");let T=L(S.chunkData);this.verifyHeader(T,"shdr"),this.dataArray.currentIndex=this.sampleDataStartIndex,this.samples.push(...Lt(T,u,!a));let O=Ue(J),q=Be(M),g=Ht(W,O,q,this.samples);this.instruments=bt(Y,g);let IA=Ue(N),$=Be(c),mA=Yt(Q,IA,$,this.instruments);this.presets.push(...Jt(f,mA,this.defaultModulators)),this.presets.sort((j,oA)=>j.program-oA.program+(j.bank-oA.bank)),x(`%cParsing finished! %c"${this.soundFontInfo.INAM}"%c has %c${this.presets.length} %cpresets,
        %c${this.instruments.length}%c instruments and %c${this.samples.length}%c samples.`,l.info,l.recognized,l.info,l.recognized,l.info,l.recognized,l.info,l.recognized,l.info),v(),a&&delete this.dataArray}verifyHeader(A,e){A.header.toLowerCase()!==e.toLowerCase()&&(v(),this.parsingError(`Invalid chunk header! Expected "${e.toLowerCase()}" got "${A.header.toLowerCase()}"`))}verifyText(A,e){A.toLowerCase()!==e.toLowerCase()&&(v(),this.parsingError(`Invalid FourCC: Expected "${e.toLowerCase()}" got "${A.toLowerCase()}"\``))}};function qe(o){let A=o.slice(8,12),e=new F(A);return V(e,4,void 0,!1).toLowerCase()==="dls "?new wA(o):new Ce(o,!1)}var ve=128,qt=147,BI=new Int16Array(qt).fill(0),gA=(o,A)=>BI[o]=A<<7;gA(H.mainVolume,100);gA(H.balance,64);gA(H.expressionController,127);gA(H.pan,64);gA(H.timbreHarmonicContent,64);gA(H.releaseTime,64);gA(H.attackTime,64);gA(H.brightness,64);gA(H.soundController6,64);gA(H.soundController7,64);gA(H.soundController8,64);gA(H.soundController9,64);gA(H.generalPurposeController6,64);gA(H.generalPurposeController8,64);gA(ve+sA.pitchWheel,64);gA(ve+sA.pitchWheelRange,2);var Ke={channelTuning:0,channelTransposeFine:1,modulationMultiplier:2,masterTuning:3,channelTuningSemitones:4},Tt=Object.keys(Ke).length,CI=new Float32Array(Tt);CI[Ke.modulationMultiplier]=1;var aa=new Uint8Array([65,16,69,18,16,0,0]);var uI={duration:99999,firstNoteOn:0,loop:{start:0,end:123456},lastVoiceEventTick:123456,lyrics:[],copyright:"",midiPorts:[],midiPortChannelOffsets:[],tracksAmount:0,tempoChanges:[{ticks:0,tempo:120}],fileName:"NOT_LOADED.mid",midiName:"Loading...",rawMidiName:new Uint8Array([76,111,97,100,105,110,103,46,46,46]),usedChannelsOnTrack:[],timeDivision:0,keyRange:{min:0,max:127},isEmbedded:!1,RMIDInfo:void 0,bankOffset:0};var FA=document.getElementById("message");document.getElementById("dls_upload").oninput=o=>{if(!o.target.files)return;let A=o.target.files[0];if(A.type.endsWith(".dls")){FA.innerText="Not a DLS file.";return}let e=document.getElementById("dls_upload_btn");e.innerText=A.name,FA.innerText="Loading...",setTimeout(async()=>{let t;try{t=qe(await A.arrayBuffer())}catch(C){FA.style.color="red",FA.innerText=`Error: ${C.message}`;return}document.getElementById("sf_info").classList.remove("hidden"),document.getElementById("dls_name").innerText=t.soundFontInfo.INAM||"Unnamed",document.getElementById("dls_description").innerText=(t.soundFontInfo.ICMT||"No description").replace(`
Converted from DLS to SF2 with SpessaSynth`,""),document.getElementById("dls_presets").innerText=t.presets.length.toString(),document.getElementById("dls_samples").innerText=t.samples.length.toString(),FA.innerText="Loaded!";let I=document.getElementById("convert");I.classList.remove("hidden");let a=document.getElementById("download"),B=a.firstElementChild;a.classList.add("hidden"),I.onclick=()=>{let C;try{C=t.write()}catch(f){FA.style.color="red",FA.innerText=`Error: ${f.message}`;return}I.classList.add("hidden"),e.innerText="Convert another DLS";let h=new Blob([C.buffer],{type:"audio/soundfont"}),u=URL.createObjectURL(h),S=A.name.replace("dls","sf2");B.href=u,B.download=S,B.innerText=`Download ${S}`,a.classList.remove("hidden"),a.innerHTML="",a.appendChild(B),FA.style.color="green",FA.innerText="Success!"}})};})();
